...
let get_andrew_primitive_datatype = function
  ...
  | DataTypes.Int16 -> AndrewLang.DataTypes.In16
  ...
  | DataTypes.Float8 -> AndrewLang.DataTypes.Flat8
...
let get_andrew_vardim_spec x y z = AndrewLang.AndrewLang.VarDimSpec (x,y,z)
...
let rec get_andrew_simple_expr = function
  ...
  | Const (x,y) -> AndrewLang.AndrewLang.Const ((get_andrew_primitive_datatype x) (get_andrew_simple_expr y))
  | VarRef x -> AndrewLang.AndrewLang.Ref (get_andrew_rsymbol x)
  | AddrRef x -> AndrewLang.AndrewLang.Ref (get_andrew_rsymbol x)
  ...
  | ColonExpr (x,y,z) -> get_andrew_vardim_spec x y z
...
and get_andrew_rsymbol = function
  ...
  | AddressedSymbol x as s -> AndrewLang.AndrewLang.RASym (get_andrew_addressed_symbol s)

and decompile_dimspecexpr = function
  | DimSpecExpr x -> get_andrew_simple_expr  x

and decompile_dimspec_list = function
...
let get_andrew_typed_address_symbol x y = 
  ...
  AndrewLang.AndrewLang.TypedAdressedSymbol ((AndrewLang.DataTypes.Aggregate (primitive_type, size_list)),(get_andrew_addressed_symbol y))
...
let get_andrew_call_list = function
  | CallAddrressedArgument x -> get_andrew_rsymbol x
...
let get_andrew_function_call outputs = function
  | Call (x,inputs)  -> AndrewLang.AndrewLang.Call ((get_andrew_symbol x),
						    ...
						    (List.map (fun x -> get_andrew_call_list x) outputs))

let get_andrew_assign_stmt expr = function
  | AllTypedSymbol x -> AndrewLang.AndrewLang.DeclAssign ((decompile_typedsymbol x), expr)
  | AllAddressedSymbol x -> AndrewLang.AndrewLang.Assign ((get_andrew_rsymbol x), expr)
  | AllSymbol x -> AndrewLang.AndrewLang.Assign ((get_andrew_rsymbol x), expr)

let decompile_simple_assign outputs expr = List.map (fun x -> (fun y -> get_andrew_assign_stmt (get_andrew_simple_expr expr)) x) outputs 
...
let rec get_andrew_stmt = function
  ...
  | Par (x,y,z) -> [AndrewLang.AndrewLang.Par((get_andrew_symbol x),(get_andrew_simple_expr y),(get_andrew_stmt z))]
  | For (x,y,z) -> [AndrewLang.AndrewLang.For((get_andrew_symbol x),(get_andrew_simple_expr y),(get_andrew_stmt z))]
  | Noop -> []
...
let get_andrew_filter = function
  | Filter (x,y,z,t) -> AndrewLang.AndrewLang.Filter ((get_andrew_symbol x),(List.map (fun x -> decompile_typedsymbol x)y),
						      (List.map (fun x -> decompile_typedsymbol x) z), get_andrew_stmt t)

let get_andrew_toplevelstmt = function
  | Def x -> AndrewLang.AndrewLang.Def (get_andrew_filter x)
