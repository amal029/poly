...
let rec codegen_simexpr declarations = function
  ...
  | Vector (dt,sa,size,lc) as s -> 
    ...
    let cvec = (const_vector (Array.init size (fun i -> (const_int (i32_type context) 0)))) in
    let () = IFDEF DEBUG THEN dump_value cvec ELSE () ENDIF in
    let ret = build_shufflevector __resvec undefvector cvec "__r1" builder in
    let () = IFDEF DEBUG THEN dump_value ret ELSE () ENDIF in ret
  ...
  | CallSymbolArgument x ->
	...
	if not defmore then
	  ...
	  let indices = List.map2 (fun x y ->
        ...
        ((Llvm_target.DataLayout.pointer_size !target_data)*8))) with
    	      | "sext" -> build_sext y
              (Llvm_target.DataLayout.intptr_type context !target_data) (if not !slots then "sexttemp" else "") builder
    	      | _ -> y)) itypes indices in
	  let () = IFDEF DEBUG THEN List.iter (fun x -> dump_value x) indices ELSE () ENDIF in
	  (* We have to do this one after the other!! *)
	  let ret = ref ret in
  ...
  | Assign (ll, expr, lc,sp) as stmt -> 
	...
	List.iter (fun x ->
	  (match x with
		...
		let shuffle_mask = codegen_simexpr !declarations !rsm in
		let () = IFDEF DEBUG THEN print_endline "Solved! the shuffle mask is!!" ELSE () ENDIF in
		let () = IFDEF DEBUG THEN dump_value shuffle_mask ELSE () ENDIF in
		...
		let vec = (derefence_pointer vptr) in
		let () = IFDEF DEBUG THEN dump_value vec ELSE () ENDIF in
		let aptr = build_bitcast vptr (pointer_type (array_type ((get_llvm_primitive_type lc vtyp) context) (vector_size (type_of vec)))) "" builder in
		let () = IFDEF DEBUG THEN dump_value aptr ELSE () ENDIF in
		...
		let temp_vec = build_bitcast temp (pointer_type (vector_type ((get_llvm_primitive_type lc vtyp) context) (vector_size (type_of rval)))) "" builder in
...
let compile optimize myarch_gpu myarch myslots vipr modules filename cfg = 
  ...
  target_data := 
    (if !march = "x86_64" then !target_data
     else Llvm_target.DataLayout.of_string "e-p:32:32:32-i1:8:8-i8:8:8-i16:16:16-i32:32:32-i64:32:32-f16:16:16-f32:32:32-v128:32:32-s:32:32-a:8:8-n32");
  ...
  (if !march = "x86_64" then
	...
	"e-p:64:64:64-i1:8:8-i8:8:8-i16:16:16-i32:32:32-i64:64:64-f32:32:32-f64:64:64-v64:64:64-v128:128:128-a0:0:64-s0:64:64-f80:128:128-n8:16:32:64" the_module
     ...
     let () = set_target_triple "i386-pc-linux-gnu" the_module in
